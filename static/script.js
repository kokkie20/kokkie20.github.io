...
function getTags(pokemon) {
	...
	if (pokemon.isShiny)
		tags.push("shiny");
	if (LEGENDS.indexOf(pokemon.dexNo) > -1)
		tags.push("legend");
	if (SUBLEGENDS.indexOf(pokemon.dexNo) > -1)
		tags.push("sublegend");
	if ((POKEMON_WITH_ALOLA_FORM.indexOf(pokemon.dexNo) > -1 && pokemon.form == "Alola Form") || POKEMON_ALLOWED_ON_VGC17.indexOf(pokemon.dexNo) > -1)
		tags.push("allowed-on-vgc17");
	if (["Normal", "Fighting", "Flying", "Poison", "Ground", "Rock", "Bug", "Ghost", "Steel", "Fire", "Water", "Grass", "Electric", "Psychic", "Ice", "Dragon", "Dark", "Fairy"].indexOf(pokemon.hiddenPower) > -1)
		tags.push("hidden-power");
	Object.keys(EGG_GROUPS).forEach(function (egg_group) {
		if (EGG_GROUPS[egg_group].indexOf(pokemon.dexNo) > -1)
			tags.push("egg-group-" + egg_group.toLowerCase().replace(' ', ''));
	});
...
function filterPokemon() {
	...
	$("tbody tr").each(function () {
		...
		if (name.indexOf(query) == -1)
			return;
		if (showOnlyBabyPokemon && !$this.hasClass("baby"))
			return;
		if (showOnlyPokemonWithHiddenAbility && !$this.hasClass("hidden-ability"))
			return;
		if (showOnlyPokemonWithViableHiddenPower && !$this.hasClass("hidden-power"))
			return;
		if (showOnlyShinyPokemon && !$this.hasClass("shiny"))
			return;
		if (showOnlyLegends && !$this.hasClass("legend"))
			return;
		if (showOnlySubLegends && !$this.hasClass("sublegend"))
			return;
		if (showOnlyPokemonAllowedOnVgc17 && !$this.hasClass("allowed-on-vgc17"))
			return;
...
function populateModal($this) {
	...
	if (notes) {
		...
		notes = notes.replace(/\[([A-zÀ-ÿ ]+)\]\((https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b(?:[-a-zA-Z0-9@:%_\+.~#?&// = ] * )) \ ) / g,
		"<a href=\"$2\">$1</a>");
	...
	$.getJSON(getWorksheetUrl(spreadsheetId, worksheetId), function (data) {
		...
		$(entry).each(function () {
			...
			for (var i = 0; i < POKE_BALLS.length; i++) {
				...
				if (tryGetValue(this, [pokeBall.replace(' ', ''), pokeBall.slice(0, -5)]))
					pokemon.balls.push(pokeBall);
			}
			if (pokemon.balls.length < 1) {
				if (getValue(this.gsx$_ddv49))
					pokemon.balls.push("Great Ball");
				if (getValue(this.gsx$_d415a))
					pokemon.balls.push("Ultra Ball");
				if (getValue(this.gsx$_d5fpr))
					pokemon.balls.push("Master Ball");
				if (getValue(this.gsx$_d6ua4))
					pokemon.balls.push("Safari Ball");
				if (getValue(this.gsx$_d88ul))
					pokemon.balls.push("Level Ball");
				if (getValue(this.gsx$_dkvya))
					pokemon.balls.push("Lure Ball");
				if (getValue(this.gsx$_dmair))
					pokemon.balls.push("Moon Ball");
				if (getValue(this.gsx$_dnp34))
					pokemon.balls.push("Friend Ball");
				if (getValue(this.gsx$_dp3nl))
					pokemon.balls.push("Love Ball");
				if (getValue(this.gsx$_df9om))
					pokemon.balls.push("Heavy Ball");
				if (getValue(this.gsx$_dgo93))
					pokemon.balls.push("Fast Ball");
				if (getValue(this.gsx$_di2tg))
					pokemon.balls.push("Sport Ball");
				if (getValue(this.gsx$_djhdx))
					pokemon.balls.push("Premier Ball");
				if (getValue(this.gsx$_dw4je))
					pokemon.balls.push("Repeat Ball");
				if (getValue(this.gsx$_dxj3v))
					pokemon.balls.push("Timer Ball");
				if (getValue(this.gsx$_dyxo8))
					pokemon.balls.push("Nest Ball");
				if (getValue(this.gsx$_e0c8p))
					pokemon.balls.push("Net Ball");
				if (getValue(this.gsx$_dqi9q))
					pokemon.balls.push("Dive Ball");
				if (getValue(this.gsx$_drwu7))
					pokemon.balls.push("Luxury Ball");
				if (getValue(this.gsx$_dtbek))
					pokemon.balls.push("Heal Ball");
				if (getValue(this.gsx$_dupz1))
					pokemon.balls.push("Quick Ball");
				if (getValue(this.gsx$_e7d2q))
					pokemon.balls.push("Dusk Ball");
				if (getValue(this.gsx$_e8rn7))
					pokemon.balls.push("Cherish Ball");
				if (getValue(this.gsx$_ea67k))
					pokemon.balls.push("Dream Ball");
				if (getValue(this.gsx$_ebks1))
					pokemon.balls.push("Beast Ball");
			}
	...
	$.getJSON(getWorksheetUrl(spreadsheetId, 1), function (data) {
		...
		$.getJSON(getSpreadsheetUrl(spreadsheetId), function (data) {
			...
			Object.keys(tabs).forEach(function (i) {
				if (tabs[i])
					$("nav > ul").append("<li><abbr title=\"" + TAB_NAMES[i] + "\">" + i + "</abbr><ul>" + tabs[i] + "</ul>");
			});
